...
void onewvel(float energy, float vel, float *vx, float *vy, float *vz, int e_flag);
float osigma1(float), osigma2(float), osigma3(float), osigma4(float), osigma5(float);
float osigma6(float), osigma7(float), osigma8(float), osigma9(float), osigma10(float);
float osigma11(float), osigma12(float), osigma13(float), osigma14(float), osigma15(float);
float osigma16(float), osigma17(float), osigma18(float), osigma19(float), osigma20(float);
float osigma21(float), osigma22(float);
float gden, vgth;
...
void oxygenmcc(int isp)
{
  ...
  static float ecol_extra,  max_sigmav_e, col_prob_e;
  static float ecol_extra2, max_sigmav_e2;
  static float ecol_extra3, max_sigmav_e3;
  static float oneg_extra,  max_sigmav_oneg, col_prob_oneg;
  static float oneg_extra2, max_sigmav_oneg2;
  static float otwo_extra,  max_sigmav_otwo, col_prob_otwo;
  static float sigma_scale;
  
  register int i, j, k, l, index;
  int s, N, nnp, foundflag;
  float sum_sigma, chi, sinchi, coschi, phi1, cosphi, sinphi;
  float temp, engy, eengy, rengy, sigma_total;
  float vneutx, vneuty, vneutz, vtempx, vtempy, vtempz;
  float del, dum, random, vel, nu_otwo_max, nu_oneg_max;
...
float osigma15(float energy)
{
  register int i;
  float detach_sigma;
  static int init_flag=1;
  static float *detach1, *detach2;
  
  /********  Initialization  ********/
  if(init_flag) {
    float engy;
    detach1 = (float *) malloc(150*sizeof(float));
    detach2 = (float *) malloc(100*sizeof(float));
...
float osigma22(float energy)
{
  int i;
  static int init_flag=1;
  static float *edetach1, *edetach2;

  /********  Initialization  ********/
  if(init_flag) {
    float engy;
    edetach1 = (float *) malloc(300*sizeof(float));
    edetach2 = (float *) malloc(432*sizeof(float));
...
void omakethefile(void)
{
  ...
  for(e=0; e<3.0; e+= .01)
    fprintf(DMPFile, "%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n", e+1e-30,
