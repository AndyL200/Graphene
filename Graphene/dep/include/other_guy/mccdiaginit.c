...
void mccdiag_init(void)
{
  ...
  if(ecollisional==0 && icollisional==0) {
	...
	if ( (gas < HELIUM) || (gas > NITROGEN) ) gas = ARGON;
	...
	case NITROGEN:
		...
		rate_title[2] = "MCC Ionization";
		rate_title[3] = "MCC Scattering";
		rate_title[4] = "MCC Charge Exchange";
		break;
  ...
  mccrate  = (float **)malloc(ndiag*sizeof(float *));
  rate_show= (float **)malloc(ndiag*sizeof(float *));
  for(i=0; i<ndiag; i++) {
    mccrate[i]  = (float *)malloc(ng*sizeof(float));
    rate_show[i]= (float *)malloc(ng*sizeof(float));
