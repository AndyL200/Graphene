...
float nsigma0(float), nsigma1(float), nsigma2(float), nsigma3(float);
float nsigma4(float), nsigma5(float), nsigma6(float);
float thrsh_engy1, thrsh_engy2, thrsh_engy3;
float thrsh_engy4, thrsh_engy5;
float gden, vgth;
...
void nnewvel(float energy, float vel, float *vx, float *vy, float *vz, float xi, int e_flag);
...
void neonmcc(int isp)
{
  ...
  if (init_flag) {
	...
	max_sigmav_e = max(max_sigmav_e, eTotalMom.sigma[i]*sqrt(eTotalMom.E[i]));
      ...
      printf("e collision prob = %g, max(sigma-v) = %g, nsigmavdt= %g\n",
	...
	max_sigmav_i= max(max_sigmav_i,
      ...
      printf("i collision prob = %g, max(sigma-v) = %g, nsigmavdt= %g\n",
...
float old_sigma6(float energy)
{
  float b2, loge, loge2, loge3, loge4, loge5;
