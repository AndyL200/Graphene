...
/* Whether Tecplot output is required or not*/
# define theRunWithTec 1

/*******************************************************************/

float nc2p, length, area, rhoback, backj, dde, epsilon, b, psi, extr,
...
float  vxu[NSMAX], vxl[NSMAX], vyu[NSMAX], vyl[NSMAX], vzu[NSMAX], vzl[NSMAX];
int    nvxbin[NSMAX], nvybin[NSMAX], nvzbin[NSMAX];

/*stuff for the volume source */
float endpts[2], vol_source, ionization_energy;

/* Tracking number of reaction events */
int N_ionize,N_elastic_el,N_excite_el,N_elastic_ion,N_exchange_ion;

/* Field emission stuff*/ // Added by Venkat on 3/2/2010
int field_emission;
float N_fe; 
float j_fe,phi_fe,beta_fe; 

/* Metal Concentration for use in the collision step */ // Added by Venkat
float metal_conc ; 

/* stuff for injection */
float eold[2], W[NSMAX], sin4W[NSMAX], sin22W[NSMAX], cos22W[NSMAX], cos_psi, sin_psi;
...
void heliummcc(int isp), argonmcc(int isp), neonmcc(int isp), oxygenmcc(int isp), mcc(int isp), nitrogenmcc(int isp);
