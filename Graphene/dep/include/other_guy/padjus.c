...
void adjust(int isp)
{
  ...
  static int sec_particles_e[2],sec_particles_i[2];
  ...
  int nreflux[NSMAX], s;
  int kchoice; //choice variable for where to introduce field emission electrons (left or right)
  float dum, del_t=0;

  float A_fe,B_fe,y_fe,v_fe,tsq_fe ;
  ...
  //printf("enna happening \n");
  if(secondary) secountl = secountr = 0;
 
  nreflux[isp]= 0;    /* nreflux[isp] equals zero */
  //printf("np[isp] = %d \n",np[isp]);
  //if (np[isp] > 0) printf("Greater than zero \n"); 
  if (np[isp] > 0) {
    //printf("inside loop np[isp] = %d",np[isp]);
    nnp = np[isp] -1;
    i = 0;
  ...
  if (inject[isp] || nreflux[isp]) {
	...
	if (np[isp] >= maxnp[isp]) {    /* Move array boundaries here */ // Condition changed from ii to np[isp] on 3/2/2010 by Venkat
...
/*	 injection_push_oldpdp(isp,ii,extra[isp][k]*del_t); */
  ...
  // Field Emission Stuff added by Venkat on 3/2/2010
  if (field_emission && isp == 0) {
    // Getting field emission current density from the electric field
    A_fe = 6.2e-6;// Fowler-Nordheim constant 1
    B_fe = 6.85e7; // Fowler-Nordheim constant 2
    
    tsq_fe = 1.1;

    // printf("%e\n",e[nc-1]);
    if (phi[nc] <= phi[0]){
      y_fe = 3.79e-4* pow(beta_fe*e[nc]*0.01,0.5)/phi_fe ; 
      v_fe = 0.95-pow(y_fe,2);
      j_fe = A_fe*pow(beta_fe*e[nc]*0.01,2)/phi_fe/tsq_fe*exp(-B_fe*pow(phi_fe,1.5)*v_fe/beta_fe/e[nc]/0.01)*1e4; // in A/m^2 due to factor of 1e4
      kchoice = 1; // introduce at right electrode since that will be cathode
    }
    else {
      //printf("%e\n",e[0]);
      y_fe = 3.79e-4* pow(beta_fe*abs(e[0])*0.01,0.5)/phi_fe ; 
      v_fe = 0.95-pow(y_fe,2);
      j_fe = A_fe*pow(beta_fe*abs(e[0])*0.01,2)/phi_fe/tsq_fe*exp(-B_fe*pow(phi_fe,1.5)*v_fe/beta_fe/abs(e[0])/0.01)*1e4; // in A/m^2 due to factor of 1e4
      kchoice = 0 ;// introduce at left electrode since that will be cathode
    }
    //printf("%e\n",j_fe);
    //j_fe = 5e6; // value set by looking at Garimella paper plot for V = 450 V and d = 5 micron ; Field = 8e7 V/m
    for(k=kchoice; k<kchoice+1; k++) { // k is used for referring to left or right electrode : k = 1 implies right ??
      N_fe = j_fe*dt/fabs(q[isp]+DBL_MIN);  // fe refers to field emission
      //printf("%d",N_fe);
      extra[isp][k] += N_fe;
      if (extra[isp][k])
	del_t = 1.0/extra[isp][k];
      while (extra[isp][k] >= 1.0) {
	extra[isp][k] -= 1.0;
	ii = np[isp];
	np[isp]++;
				
	if (np[isp] >= maxnp[isp]) {    /* Move array boundaries here */ 

	  printf("ADJUST - Field Emission: too many particles, species %d",isp);
	  exit(1);
	}
...
/*	 injection_push_oldpdp(isp,ii,extra[isp][k]*del_t); */
  ...
  if (isp == 0){
    sec_particles_e[0] += secountl;
    sec_particles_e[1] += secountr;
  }
  if (isp == 1){
    sec_particles_i[0] += secountl;
    sec_particles_i[1] += secountr;
  }
  if (remainder(it[0],1000) == 0){
    if (isp == 0){
      second = fopen("secondary_el.plt","a");
      fprintf(second,"%e %d %d\n",t,sec_particles_e[0],sec_particles_e[1]);
      fclose(second);
    }
    if (isp == 1){
      second = fopen("secondary_ion.plt","a");
      fprintf(second,"%e %d %d\n",t,sec_particles_i[0],sec_particles_i[1]);
      fclose(second);
    }
  }
} /* end ADJUST */
