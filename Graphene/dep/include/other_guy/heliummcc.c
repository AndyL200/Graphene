...
void hnewvel(float, float, float *, float *, float *, int);
float hsigma1(float), hsigma2(float), hsigma3(float), hsigma4(float);
...
void heliummcc(int isp)
{
  ...
  if(icollisional && isp==icolsp) {
    /**** First Clear the diagnostics  ******/
    if(theRunWithXFlag) {
      for (j=0; j<ng; j++) mccrate[3][j] = 0.0;
    }
    /**** Now do the collisions *****/
    ...
    for(j=nnp; j<nnp+N; j++) {
      ...
      dum = (vx[icolsp][j]*vx[icolsp][j] +vy[icolsp][j]*vy[icolsp][j]
			...
			else {/*particle does charge exchange*/
				sigma_total = 1e7;
				random = 0;
			}
			...
			if (random <= hsigma4(engy)/sigma_total) {
...
void hnewvel(float energy, float vel, float *vx, float *vy, float *vz, int e_flag)
{
