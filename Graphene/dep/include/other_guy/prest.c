...
void realft(float data[], int, int);
void velocity(void), freqanalysis(void);
float bit_rever_maxwell(), maxwell(void);
...
float distribution_flux(int k, int isp, int type)
{
	static int init=1;
	static int cutoff=1;
	static int init_sp[NSMAX][2];
...
void history(void)
{
  ...
  if (n_ave){
	...
	for(i=0; i<ng; i++) {
	  ...
	  sp_j_x_ave_show[isp][i] = sp_j_x_ave[isp][i]/n_ave*j_scale[isp]; sp_j_x_ave[isp][i] = sp_j_x[isp][i]; // j_scale factor added by Venkat on 1/21/2009
	  sp_j_y_ave_show[isp][i] = sp_j_y_ave[isp][i]/n_ave*j_scale[isp]; sp_j_y_ave[isp][i] = sp_j_y[isp][i]; // j_scale factor added by Venkat on 1/21/2009
	  sp_j_z_ave_show[isp][i] = sp_j_z_ave[isp][i]/n_ave*j_scale[isp]; sp_j_z_ave[isp][i] = sp_j_z[isp][i]; // j_scale factor added by Venkat on 1/21/2009
	}
	for (i=0; i<nbin_mid[isp]; i++){
